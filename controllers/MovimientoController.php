...
class MovimientoController {
    ...
    public function exportar() {
        try {
            ...
            if (empty($movimientos)) {
                header('Location: index.php?controller=movimientos&action=index&error=sin_datos');
                exit();
            }
            ...
            require_once 'includes/ExcelGeneratorSimple.php';
            require_once 'includes/PDFGenerator.php';
            
            $filename = 'movimientos_' . date('Y-m-d_H-i-s');
            
            switch ($formato) {
                case 'excel':
                    try {
                        ExcelGenerator::generarXLSX($movimientos, $filename . '.xlsx');
                    } catch (Exception $e) {
                        // Si falla la versión completa, usar la versión simple
                        ExcelGeneratorSimple::generarXLSX($movimientos, $filename . '.csv');
                    }
                    break;
                case 'pdf':
                    PDFGenerator::generarPDF($movimientos, $filename . '.pdf');
                    break;
                default:
                    try {
                        ExcelGenerator::generarXLSX($movimientos, $filename . '.xlsx');
            ...
            header('Location: index.php?controller=movimientos&action=index&error=exportar_fallo');
            exit();
        }
    }
